...
fake_news_filt <- fake_news %>% 
  select(type, title_has_excl)

fake_news_filt <- fake_news %>% 
  group_by(title_has_excl, type) %>% 
  count()

glimpse(fake_news_filt)

#ONE HOT ENCODING 
dummy <- dummyVars(" ~ .", data=fake_news_filt)

encoded_df <- data.frame(predict(dummy, newdata=fake_news_filt))

ggplot(fake_news, aes(type, fill= title_has_excl)) +
  geom_bar() +
  labs(title = "Title", x="hi", fill = "legend")
...
-   The prior probability (an [unconditional probability](https://www.bayesrulesbook.com/chapter-2#building-a-bayesian-model-for-events:~:text=two%20events.%20The-,unconditional%20probability,-of)) that the article is fake is 0.4
    ...
    ```{r}
    prior_probability_article_fake <- 0.4 #unconditonal, not counting for other data
    ```

P(Ac) = 0.88

-   The prior probability (an unconditional probability) that the article is real is 0.88
...
-   The prior probability (an [unconditional probability](https://www.bayesrulesbook.com/chapter-2#building-a-bayesian-model-for-events:~:text=two%20events.%20The-,unconditional%20probability,-of)) that the article has an exclamation point is 0.12
    ...
    ```{r}
    prior_probability_has_exc <- 0.12 #unconditonal 
    ```
...
```{r}
likelihood_article_fake_has_exc <- .27
```
...
```{r}
prior_probability_southern_citizen_S_A <- 0.24
prior_probability_says_pop_pa <- 0.37
likelihood_says_pop <- 0.05
```

```{r}
prior_probability_southern_citizen_S_A * likelihood_says_pop/
prior_probability_says_pop_pa
```
...
```{r}
prior_probability_says_pop <- sum(prior*likelihoods)
...
```{r}
(prior[3] * likelihoods[3]) / prior_probability_says_pop
```
